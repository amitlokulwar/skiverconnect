<?php
    ...
    while ( have_posts() ): the_post();
		...
		$package = get_the_terms( $item_id, 'package-days');
		$packageDays = $package[0]->name;
		if($packageDays){
			$end = date('d/m/Y', strtotime($start. ' + '.$packageDays.' days'));
			$init = date( 'd/m/Y h:i a', strtotime($current_calendar));
			$date = STInput::get( 'date', date( 'd/m/Y h:i a', strtotime($current_calendar)) . '-' . date( 'd/m/Y h:i a',strtotime($init. ' + '.$packageDays.' days') ) );
		}else{
                                                            ...
                                                            foreach ( $facilities as $term ) {
                                                                ...
                                                                <div class="col-xs-6 col-sm-4">
                                                                    <div class="item has-matchHeight">
                                                                        <?php
                                                                            if ( !$icon_new ) {
																				if ($get_label_tax->label == 'Departure day' || $get_label_tax->label == 'Package days' ){
																					echo '<i class="' . esc_attr($icon) . '"></i>' . esc_html($term->description);
